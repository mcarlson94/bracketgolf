...
import { CreateBracketModal } from "@/components/bracket/CreateBracketModal";
...
export default function Dashboard() {
  ...
  const openModal = (round: "R64" | "R32") => {
    setDefaultRound(round);
    setModalOpen(true);
  };

  const handleBracketCreated = (bracketId: string) => {
    setLocation(`/brackets/${bracketId}`);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <CreateBracketModal
        ...
        defaultStartRound={defaultRound}
        ...
        <div className="relative z-10">
          ...
          {!isLocked && (
            <div className="flex flex-wrap gap-3">
              <Button
                size="lg"
                onClick={() => openModal("R64")}
                disabled={!tournament || tournament.status === "upcoming"}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {loadingBrackets ? (
            ...
            <div className="space-y-4">
              {brackets.map((bracket) => (
                <Card
                  key={bracket.id}
                  className="hover:shadow-md transition-shadow border-l-4 border-l-primary overflow-hidden relative group"
                >
                        ...
                        <div className="flex justify-between text-xs mb-1">
                          <span className="font-medium flex items-center gap-1.5">
                            {bracket.startRound === "R32" && (
                              <span className="text-[10px] font-bold bg-primary/10 text-primary px-1.5 py-0.5 rounded-sm uppercase tracking-wide">
                                R32
                              </span>
                            )}
                          ...
                          <span className="text-muted-foreground">
                            {Math.round((bracket.completedPicks / bracket.totalPicks) * 100)}%
                          </span>
                        </div>
                        <Progress
                          value={(bracket.completedPicks / bracket.totalPicks) * 100}
                          className="h-2"
                        />
                      </div>
            ...
            <Card className="p-12 text-center border-dashed border-2">
              ...
              <p className="text-muted-foreground mb-6">
                Create your first bracket and start making your picks.
              </p>
              <Button
                onClick={() => openModal("R64")}
                disabled={!tournament || tournament.status === "upcoming"}
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold font-heading text-foreground">Groups</h2>
            <Link href="/groups">
